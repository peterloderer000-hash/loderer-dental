...
export default function AuthScreen() {
  ...
  async function handleSignIn() {
    if (!email || !password) {
      Alert.alert('Chyba', 'Vyplňte email aj heslo.');
      return;
    }
    setLoading(true);
  ...
  async function handleSignUp() {
    ...
    setLoading(false);
    if (error) {
      Alert.alert('Chyba registrácie', error.message);
    } else {
      router.push('/setup-role');
    }
  }
...
const styles = StyleSheet.create({
  ...
  label: {
    fontSize: 11, fontWeight: '500', color: COLORS.wal,
  ...
  input: {
    ...
    fontSize: 15, color: COLORS.esp, backgroundColor: '#fff',
  },
  loader: { marginTop: 24 },
  btnPrimary: {
    backgroundColor: COLORS.esp,
    borderWidth: 2.5, borderColor: COLORS.wal,
  ...
  btnSecondary: {
    ...
    shadowColor: COLORS.wal, shadowOffset: { width: 0, height: 4 },
