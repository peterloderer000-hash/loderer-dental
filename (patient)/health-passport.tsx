...
// ─── Sekcia 8: Životný štýl ──────────────────────────────────────────────────
const LIFESTYLE_OPTIONS = ['Fajčenie', 'Káva', 'Víno'];
// ─── Sekcia 9: Investičné očakávania ─────────────────────────────────────────
...
// ─── Helper: prepnúť položku v poli ──────────────────────────────────────────
function toggle(arr: string[], val: string): string[] {
  return arr.includes(val) ? arr.filter((v) => v !== val) : [...arr, val];
}

// ─── Komponent: Checkbox (multi-výber) ───────────────────────────────────────
...
export default function HealthPassportScreen() {
  ...
  // Načítaj existujúce dáta pri otvorení (edit-mode)
  useEffect(() => {
        ...
        <ScrollView style={styles.scroll} contentContainerStyle={styles.content}
          ...
          <View style={styles.card}>
            {VISIT_REASONS.map((item) => (
              <CheckItem key={item} label={item}
                selected={visitReasons.includes(item)}
                onToggle={() => setVisitReasons((p) => toggle(p, item))} />
            ))}
          </View>

          {/* ── 3. Medicínska anamnéza ── */}
          ...
          <View style={styles.card}>
            ...
            {MEDICAL_CONDITIONS.map((item) => (
              <CheckItem key={item} label={item}
                selected={medConditions.includes(item)}
          ...
          <View style={styles.card}>
            {FEAR_LEVELS.map((item) => (
              <RadioItem key={item} label={item}
                selected={fearLevel === item}
                onSelect={() => setFearLevel(item)} />
            ))}
          </View>

          {/* ── 6. Komfort počas ošetrenia ── */}
          ...
          <View style={styles.card}>
            ...
            {COMFORT_OPTIONS.map((item) => (
              <RadioItem key={item} label={item}
                selected={comfort === item}
                onSelect={() => setComfort(item)} />
            ))}
          </View>

          {/* ── 7. Estetické očakávania ── */}
          ...
          <View style={styles.card}>
            ...
            {AESTHETIC_OPTIONS.map((item) => (
              <CheckItem key={item} label={item}
                selected={aesthetics.includes(item)}
          ...
          <View style={styles.card}>
            <Text style={styles.cardSub}>
              Čo by sme mohli urobiť, aby bola vaša návšteva čo najpríjemnejšia?
            </Text>
          ...
          <TouchableOpacity style={[styles.saveBtn, saving && styles.saveBtnDisabled]}
            ...
            {saving
              ...
              : (<><Ionicons name="checkmark-circle-outline" size={18} color="#fff" />
                  <Text style={styles.saveBtnText}>Uložiť dotazník</Text></>)}
          </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  header: {
    ...
    paddingHorizontal: SIZES.padding, paddingTop: 14, paddingBottom: 18,
  ...
  introBanner: {
    ...
    backgroundColor: COLORS.bg3, borderRadius: SIZES.radius,
    borderLeftWidth: 3, borderLeftColor: COLORS.sand,
    padding: 12, margin: SIZES.padding, marginBottom: 4,
  },
  ...
  card: {
    ...
    marginHorizontal: SIZES.padding, padding: 14,
    borderWidth: 1, borderColor: COLORS.bg3, gap: 6,
  },
  ...
  option: {
    ...
    borderRadius: 8, borderWidth: 1, borderColor: COLORS.bg3,
  ...
  input: {
    backgroundColor: COLORS.bg2, borderWidth: 1.5, borderColor: COLORS.bg3,
    borderRadius: SIZES.radius - 2, padding: 10,
    fontSize: 13, color: COLORS.esp, minHeight: 60, lineHeight: 20,
  },
  ...
  saveBtn: {
    backgroundColor: COLORS.wal, borderRadius: SIZES.radius,
    paddingVertical: 15, marginHorizontal: SIZES.padding, marginTop: 20,
    alignItems: 'center', justifyContent: 'center',
    flexDirection: 'row', gap: 8,
    shadowColor: COLORS.esp, shadowOffset: { width: 0, height: 3 },
